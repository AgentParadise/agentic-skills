...
`/Users/yourname/...` or `~/Code/...` in a cross-reference. Breaks the moment the skill is installed in a different location. Cross-references inside a skill use paths relative to the skill root (`references/foo.md`), and cross-references between skills use the plugin-qualified form documented in `anthropic-skill-creator.md`.
