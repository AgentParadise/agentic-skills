...
skip to the raw `codex exec` recipes below, which keep the same sandbox rules.
...
The fix belongs to the workspace, not the invocation. In Agentic Workspace
images, `agentic_isolation` applies the Codex sandbox policy to every image labelled
